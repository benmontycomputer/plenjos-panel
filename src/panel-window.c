...
void update_desktop_pixbuf(PanelWindow *self)
{
  ...
  panel_applications_menu_set_bg(self->apps_menu, self->desktop_blurred);
}
...
{
  (void)widget;
  if (GDK_IS_WINDOW(event->window) && (event->x != ((self->monitor_geometry.width) - event->width) / 2 || event->y != self->monitor_geometry.height - event->height))
  {
